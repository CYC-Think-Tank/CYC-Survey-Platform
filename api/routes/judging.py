...
async def get_judging_surveys(judge_id: str):
    ...
    try:
        ...
        surveys_res = (
            supabase.table("surveys").select("*").eq("is_active", True).execute()
        )
        ...
        scores_res = (
            ...
            .select("*")
            .eq("analysis_type", f"judge_score_{judge_id}")
            .execute()
        )
...
async def get_auto_scores(survey_id: str):
    ...
    try:
        ...
        total_responses = (
            sessions_res.count
            if hasattr(sessions_res, "count")
            else len(sessions_res.data or [])
        )
        languages = set(
            [s.get("language") for s in (sessions_res.data or []) if s.get("language")]
        )
        ...
        # 2. Geographic Score
        q_res = (
            supabase.table("questions").select("*").eq("survey_id", survey_id).execute()
        )
        geo_score = 0
        valid_provs = 0
        for q in q_res.data or []:
            ...
            if (
                ...
                and opts.get("validation", {}).get("type") == "postal_code_prefix"
            ):
                ans_res = (
...
async def submit_score(score: JudgeScore):
    ...
    try:
        ...
        existing_res = (
            supabase.table("ai_analyses")
            .select("id")
            .eq("survey_id", score.survey_id)
            .eq("analysis_type", f"judge_score_{score.judge_id}")
            .execute()
        )
        if existing_res.data:
            # Update
            supabase.table("ai_analyses").update(payload).eq(
