...
export default function EditSurvey() {
  ...
  const populateTranslations = async () => {
    ...
    if (language === 'fr') {
      ...
      if (transData.description_fr && transData.description_fr.trim()) setDescriptionFr(transData.description_fr);
      if (transData.questions_fr) {
        setQuestions(prev => prev.map((q, idx) => {
          ...
          if (translated.options && translated.options.description !== undefined && translated.options.description.trim()) {
      ...
      if (transData.description_zh && transData.description_zh.trim()) setDescriptionZh(transData.description_zh);
      ...
      .then(([data, transData]) => {
        ...
        const loadedQuestions = data.questions.map((q: any) => {
          ...
          return {
            ...
            question_description: (q.type === 'short_answer' && !isArr) ? (q.options.description || '') : '',
            validation_type: (q.type === 'short_answer' && !isArr && q.options.validation) ? (q.options.validation.type || 'none') : 'none',
            validation_regex: (q.type === 'short_answer' && !isArr && q.options.validation) ? (q.options.validation.regex || '') : '',
            validation_max_length: (q.type === 'short_answer' && !isArr && q.options.validation) ? q.options.validation.max_length : undefined,
            validation_normalize_uppercase: (q.type === 'short_answer' && !isArr && q.options.validation) ? (q.options.validation.normalize_uppercase || false) : false,
            ...
            question_description_fr: (q.type === 'short_answer' && !isFrArr && frQ.options?.description) ? frQ.options.description : '',
            ...
            question_description_zh: (q.type === 'short_answer' && !isZhArr && zhQ.options?.description) ? zhQ.options.description : '',
          };
        });
        ...
        <div className="card space-y-4">
          <div>
            ...
            <input
              ...
              placeholder={language === 'fr' ? "Titre en francais" : language === 'zh' ? "中文标题" : "Survey Title"}
            />
          </div>
          <div>
          ...
          {questions.map((q, qIdx) => {
            ...
            return (
                    ...
                    {optionsArray.map((opt: string, oIdx: number) => (
                      <div key={oIdx} className="flex items-center space-x-2">
                        ...
                        <input
                          ...
                          value={opt}
                          required={language === 'en'}
                          placeholder={language === 'fr' ? (getOptionsArray(q.options)[oIdx] || `Option ${oIdx + 1} (Francais)`) : language === 'zh' ? (getOptionsArray(q.options)[oIdx] || `选项 ${oIdx + 1}`) : `Option ${oIdx + 1}`}
                  ...
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <RichTextEditor
                        ...
                        readOnly={isLocked && language === 'en'}
                        placeholder={language === 'en' ? "Provide context or instructions before the next set of questions..." : language === 'fr' ? "Traduction française du contexte..." : "中文描述..."}
                      />
                    </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                ...
                {(language === 'en' ? q.definitions : language === 'fr' ? (q.definitions_fr || q.definitions) : (q.definitions_zh || q.definitions)) && (language === 'en' ? q.definitions : language === 'fr' ? (q.definitions_fr || q.definitions) : (q.definitions_zh || q.definitions))!.length > 0 && (
                    ...
                    {(language === 'en' ? q.definitions : language === 'fr' ? (q.definitions_fr || q.definitions) : (q.definitions_zh || q.definitions))!.map((def, dIdx) => (
                      <div key={dIdx} className="flex items-start space-x-2">
                        <div className="w-1/3">
                          <input
                            type="text"
                            placeholder={language === 'fr' ? (q.definitions![dIdx]?.term || "Terme") : language === 'zh' ? (q.definitions![dIdx]?.term || "术语") : "Term"}
                            value={def.term}
