        ...
        <div className="card space-y-4">
          <div>
            ...
            <input
              ...
              placeholder={
                language === 'en' ? 'e.g. Mental Health Perspectives 2026' : `Title in ${language}`
              }
            />
          </div>
          <div>
        ...
        <div className="space-y-6">
          ...
          {language !== 'en' && (
            <div className="bg-blue-50 dark:bg-slate-800/50 text-blue-600 dark:text-blue-400 p-3 rounded-lg text-sm flex items-start">
              ...
              <p>
                <strong>Translation Mode:</strong> Structural changes (adding/deleting questions or
                options) are disabled while translating. Switch back to English to modify the survey
                structure.
              </p>
            </div>
          )}
          ...
          {questions.map((q, qIdx) => (
            ...
            >
              <div
                className={`absolute top-4 right-4 flex items-center space-x-1 transition-opacity ${language !== 'en' ? 'hidden' : 'opacity-0 group-hover:opacity-100'}`}
              >
                <button
                  type="button"
              ...
              <div className="flex items-start space-x-4 mb-4 pr-28">
                <span className="font-bold text-gray-400 dark:text-slate-500 mt-8 w-6 text-right">
                  ...
                  <input
                    ...
                    value={
                      language === 'en'
                        ...
                        : language === 'fr'
                          ? q.question_description_fr || ''
                          : q.question_description_zh || ''
                    }
                    onChange={(e) =>
                      updateQuestion(
                        q.id,
                        ...
                        {q.validation_type === 'regex' ? (
                          ...
                          <div className="sm:col-span-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 text-sm text-blue-700 dark:text-blue-300">
                            ...
                            {q.validation_type === 'postal_code_prefix' && (
                              <>
                                <strong>Postal code prefix rules:</strong> Max 3 characters. Format:
                                letter, number, letter (e.g. M5V). Input is automatically normalized
                                to uppercase.
                              </>
                            )}
                          </div>
                ...
                {q.type === 'checkboxes' && (
                  <label className="flex items-center">
                    ...
                    <input
                      type="number"
                      min={1}
                      max={q.options.length + (q.has_other ? 1 : 0)}
              ...
              {(q.type === 'multiple_choice' ||
                ...
                <div className="ml-10 pr-28 space-y-2">
                  {getOptionsForDisplay(q).map((opt, oIdx) => (
                    <div key={oIdx} className="flex items-center space-x-2">
                      ...
                      <button
                        ...
                        className={`ml-2 ${(q.locked_choices || []).includes(opt) ? 'text-[var(--color-cyc-primary)]' : 'text-gray-300 hover:text-gray-500 dark:text-slate-500'} ${language !== 'en' ? 'hidden' : ''}`}
                      ...
                      {getOptionsForDisplay(q).length > 1 && (
                        <button
                          ...
                          className={`text-gray-400 dark:text-slate-500 hover:text-red-500 ${language !== 'en' ? 'hidden' : ''}`}
                        >
                          &times;
                        </button>
                      )}
                  ...
                  {q.has_other && (
                    <div className="flex items-center space-x-2 opacity-60">
                      <div
                        className={`w-4 h-4 border border-gray-400 ${q.type === 'multiple_choice' || q.type === 'dropdown' ? 'rounded-full' : 'rounded'}`}
                      />
                      <span className="text-sm text-gray-500 dark:text-slate-500 italic">
                        Other: ____
                      </span>
                    </div>
                  )}
                  <button
                    ...
                    className={`text-sm text-[var(--color-cyc-primary)] hover:underline mt-2 inline-block ${language !== 'en' ? 'hidden' : ''}`}
                  >
                    + Add Option
                  </button>
                </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    className={`text-xs text-[var(--color-cyc-primary)] hover:underline ${language !== 'en' ? 'hidden' : ''}`}
                  >
                    + Add Definition
                  </button>
                </div>
                    ...
                    {(language === 'en'
                      ...
                      <div key={dIdx} className="flex items-start space-x-2">
                        ...
                        <button
                          ...
                          className={`text-gray-400 dark:text-slate-500 hover:text-red-500 mt-1 ${language !== 'en' ? 'hidden' : ''}`}
                        >
                          &times;
                        </button>
                      </div>
                      ...
                      >
                        <div className="flex-grow flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                          ...
                          >
                            ...
                            {gate.question_id && questions.find((pq) => pq.id === gate.question_id)
                              ? getOptionsArray(
                        ...
                        <button
                          ...
                          className={`p-1.5 text-gray-400 hover:text-red-500 ${language !== 'en' ? 'hidden' : ''}`}
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
          ...
          <div
            className={`bg-gray-50 dark:bg-slate-900/50 border-2 border-dashed border-gray-300 dark:border-slate-600 rounded-xl p-6 text-center ${language !== 'en' ? 'hidden' : ''}`}
          >
