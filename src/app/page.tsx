...
export default function Home() {
  ...
  const decos = [
    // Far Background (Small, blurry, low opacity)
    {
      ...
      cls: 'top-[25%] left-[8%] rotate-[-15deg] scale-75 opacity-30 blur-[2px] z-0',
      bg: 'bg-[#04377E]',
      delay: 0,
    },
    {
      icon: <Users className="w-5 h-5 text-white" />,
      cls: 'bottom-[35%] right-[10%] rotate-[10deg] scale-50 opacity-20 blur-[3px] z-0',
      bg: 'bg-[#0CB7C4]',
      delay: 1.2,
    },
    ...
    // Midground (Normal size, sharp, medium opacity)
    {
      ...
      cls: 'top-[50%] left-[5%] rotate-[25deg] scale-100 opacity-60 z-0',
      bg: 'bg-[#0CB7C4]',
      delay: 0.5,
    },
    {
      icon: <Star className="w-5 h-5 text-[#04377E]" />,
      cls: 'top-[15%] right-[15%] rotate-[-20deg] scale-90 opacity-50 z-0',
    ...
    // Foreground (Large, sharp, high opacity, pulled slightly inward to prevent clipping)
    {
      ...
      cls: 'bottom-[12%] left-[6%] rotate-[-18deg] scale-125 opacity-90 z-20',
      bg: 'bg-[#F5C518] shadow-xl',
      delay: 0.8,
    },
    {
      icon: <BarChart3 className="w-6 h-6 text-white" />,
      cls: 'top-[70%] right-[6%] rotate-[30deg] scale-150 opacity-80 z-20',
      bg: 'bg-[#04377E] shadow-xl',
      delay: 2.5,
    },
  ];
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute top-[22%] right-[16%] w-4 h-4 rounded-full bg-[#04377E] opacity-20 blur-[2px] hidden md:block z-0"
        initial={{ opacity: 0 }}
        animate={{ opacity: showIntro ? 0 : 0.2 }}
        transition={{ duration: 1.2, delay: 0.5 }}
      />
      <motion.div
        className="absolute bottom-[30%] left-[8%] w-6 h-6 rounded-full bg-[#0CB7C4] opacity-30 blur-[1.5px] hidden md:block z-0"
        initial={{ opacity: 0 }}
        animate={{ opacity: showIntro ? 0 : 0.3 }}
        transition={{ duration: 1.2, delay: 0.6 }}
      />
      <motion.div
        className="absolute top-[65%] left-[22%] w-8 h-8 rounded-full border-[3px] border-[#F5C518] opacity-25 blur-[2px] hidden md:block z-0 scale-75"
        initial={{ opacity: 0 }}
        animate={{ opacity: showIntro ? 0 : 0.25 }}
        transition={{ duration: 1.2, delay: 0.7 }}
      />

      {/* Raffle Info Floating Box */}
      <motion.div
        className="absolute top-[8%] md:top-[10%] lg:top-[12%] right-[-15px] md:right-[-10px] lg:right-[-5px] xl:right-[1%] 2xl:right-[4%] z-50 hidden md:flex flex-col items-center rotate-[3deg] cursor-pointer"
        initial={{ opacity: 0, scale: 0 }}
          ...
          {items.map((item, idx) => {
            ...
            return (
              <div
                ...
                style={{
                  transform: `translateX(${x}px) translateY(${yOff}px) scale(${sc}) rotateZ(${rotZ}deg) rotateX(${rotX}deg)`,
                  zIndex: zI,
                }}
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className="relative z-10 p-4 sm:p-5 md:p-8 flex flex-col h-full justify-between"
                    ...
                    >
                      <div>
                        ...
                        <h2 className="text-lg sm:text-2xl md:text-4xl font-extrabold mb-1.5 sm:mb-3 leading-tight line-clamp-2 md:line-clamp-3 text-[#1a1a1a] drop-shadow-sm">
                        ...
                        <p className="text-[11px] sm:text-sm md:text-base leading-relaxed line-clamp-2 sm:line-clamp-3 md:line-clamp-4 text-gray-500">
                          {(item.displayDescription || item.description)?.replace(
                            /<[^>]*>?/gm,
                            ''
                      ...
                      <div className="mt-auto flex items-center justify-end">
                        {item.isComingSoon ? (
                          <button
                            disabled
                            className="flex items-center px-4 py-1.5 sm:px-6 sm:py-2.5 rounded-full text-xs sm:text-base font-bold bg-gray-100 text-gray-400 cursor-not-allowed"
                          >
                            {t('Coming Soon')}
                          </button>
                        ) : isCompleted ? (
