...
export default function SurveyPage() {
  ...
  const _referralSource = searchParams.get('ref') || null;
  const [survey, setSurvey] = useState<any>(null);
  const [alreadyCompleted, setAlreadyCompleted] = useState(false);
  const [hasStarted, setHasStarted] = useState(false);
  const [currentStep, setCurrentStep] = useState(0); // 0 = email, 1+ = questions
  const [email, setEmail] = useState('');
  const [profileData, _setProfileData] = useState<Record<string, any>>({});
  ...
  const _saveCurrentAnswer = async (sid: string, questionIdx: number) => {
  ...
  async function handleNext() {
    ...
    if (!isEmailStep && currentQuestion) {
      ...
      if (currentQuestion.id.startsWith('attn-')) {
        ...
        if (currentQuestion.id === 'attn-fixed-2' && (val === 'False' || val === 'Faux'))
          passed = true;
        if (currentQuestion.id === 'attn-inact-1' && (val === 'Yes' || val === 'Oui'))
          passed = true;
